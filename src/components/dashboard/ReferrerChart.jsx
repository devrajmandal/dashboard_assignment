...
const Bar = ({ height, value, label, active }) => (
  <div className="flex flex-col items-center justify-end h-full gap-3 w-16 relative group">
    <div className={`text-[10px] font-bold text-pink-500 bg-white px-2.5 py-1.5 shadow-md rounded-lg absolute bottom-full mb-3 whitespace-nowrap transition-opacity ${active ? 'opacity-100' : 'opacity-0 group-hover:opacity-100'}`}>
      {value}
    </div>
  ...
  <div className="col-span-5 bg-white rounded-[32px] p-8 shadow-sm border border-white h-full">
    <div className="flex items-center justify-between mb-8">
      <div className="flex items-center gap-1 text-gray-300">
        <BarChart3 size={20}/><ChevronDown size={14}/>
      </div>
